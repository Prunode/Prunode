...
╹  ╹┗╸┗━┛╹ ╹┗━┛╺┻┛┗━╸
    ...
    console.log(
      chalk.gray.italic(`      Enterprise Cross-Boundary Resolution Engine\n`),
    );
  }

  const config = { ignore: ['node_modules', 'dist', '.git', '.next'] };
  const configPath = path.join(scanDir, 'prunode.json');
  if (fs.existsSync(configPath)) {
    const userConfig = JSON.parse(fs.readFileSync(configPath, 'utf8'));
    config.ignore = [...config.ignore, ...(userConfig.ignore || [])];
    console.log(
      chalk.yellow(`[⚙️] Loaded custom configuration from prunode.json`),
    );
  }
  ...
  async function getFiles(currentDir: string): Promise<string[]> {
    const dirents = await fsPromises.readdir(currentDir, {
      withFileTypes: true,
    });
  ...
  console.log(
    chalk.blueBright(`[2/3] AST Compilation Complete. `) +
      chalk.gray(
        `Found ${graph.routes.length} backend endpoints, ${graph.calls.length} frontend queries.`,
      ),
  );
            ...
            locations: [
              {
                physicalLocation: {
                  artifactLocation: {
                    uri: path.relative(process.cwd(), r.sourceFile),
                  },
                },
              },
            ],
  ...
  if (options.html) {
    ...
    console.log(
      chalk.gray(`   Open it in your browser to explore your endpoints.`),
    );
  }
    ...
    for (const [file, routes] of grouped.entries()) {
      ...
      for (const r of routes) {
        if (r.startPos !== undefined && r.endPos !== undefined) {
          content =
            content.substring(0, r.startPos) + content.substring(r.endPos);
        }
      }
  ...
  const table = new Table({
    head: [
      ...
      chalk.bold.white('Endpoint Route'),
      chalk.bold.white('Payload Schema'),
      chalk.bold.white('Dead Fields'),
      chalk.bold.white('Components'),
  ...
  activeRoutes.forEach((r) => {
    const payloadStr =
      r.payloadKeys.length > 0
        ? chalk.cyan(`{ ${r.payloadKeys.join(', ')} }`)
        : chalk.gray('{}');
    const deadFieldsStr =
      r.deadFields && r.deadFields.length > 0
        ? chalk.yellow(`{ ${r.deadFields.join(', ')} }`)
        : chalk.gray('-');
    const componentsStr =
      r.usedByComponents && r.usedByComponents.length > 0
        ? chalk.magenta([...new Set(r.usedByComponents)].join(', '))
        : chalk.gray('-');
    if (r.deadFields && r.deadFields.length > 0)
      totalDeadFields += r.deadFields.length;
  ...
  deadRoutes.forEach((r) => {
    ...
    if (r.startPos !== undefined) {
      try {
        ...
        const blame = execSync(
          `git blame -L ${lineNum},${lineNum} --porcelain ${r.sourceFile}`,
          { stdio: 'pipe' },
        ).toString();
    ...
    table.push([
      chalk.redBright(r.method),
      chalk.red(r.path),
      payloadStr,
      chalk.gray('-'),
      chalk.gray('-'),
      chalk.yellow(author),
      chalk.red.bold('ORPHAN / DEAD'),
    ]);
  });
...
╹  ╹┗╸┗━┛╹ ╹┗━┛╺┻┛┗━╸
  ...
  console.log(
    chalk.gray.italic(`      Prunode Interactive Terminal Session\n`),
  );
  console.log(
    chalk.gray(
      `Type ${chalk.cyanBright('scan .')} to run the AST Engine on the current directory.\n`,
    ),
  );
  ...
  rl.on('line', async (line) => {
    ...
    try {
      if (cmd === 'exit' || cmd === 'quit') {
        ...
        const targetDir =
          args[1] || process.env.HOME || process.env.USERPROFILE || '.';
        try {
          process.chdir(targetDir);
          updatePrompt();
        } catch (e) {
          console.error(
            chalk.red(`cd: ${targetDir}: No such file or directory`),
          );
        }
      } else if (cmd === 'scan') {
        ...
        const targetDir = args[1];
        if (targetDir) {
          fs.mkdirSync(path.resolve(process.cwd(), targetDir), {
            recursive: true,
          });
        } else {
...
program
  ...
  .option(
    '--sarif',
    'Output results in SARIF format for GitHub Advanced Security',
  )
...
program
  ...
  .action(async (dir) => {
    ...
    server.listen(3000, () => {
      ...
      console.log(
        chalk.white(`   Open `) +
          chalk.cyan.bold(`http://localhost:3000`) +
          chalk.white(` in your browser.`),
      );
