...
export function activate(context: vscode.ExtensionContext) {
  ...
  let disposable = vscode.commands.registerCommand('prunode.scan', () => {
    ...
    vscode.window.showInformationMessage(
      'Prunode: Scanning for dead endpoints...',
    );
      ...
      (error, stdout, stderr) => {
        ...
        if (!fs.existsSync(sarifPath)) {
          vscode.window.showErrorMessage(
            'Prunode: SARIF file not found. Ensure prunode is installed globally.',
          );
          return;
        }
        ...
        vscode.window.showInformationMessage(
          `Prunode: Found ${sarifData.runs[0].results?.length || 0} orphaned endpoints.`,
        );
      },
    );
  });
  ...
  const quickFixProvider = vscode.languages.registerCodeActionsProvider(
    { scheme: 'file' },
    {
      provideCodeActions(
        ...
        const prunodeDiagnostics = context.diagnostics.filter((d) =>
          d.message.includes('Dead Backend Endpoint'),
        );
        if (prunodeDiagnostics.length === 0) return [];

        const fix = new vscode.CodeAction(
          '✂️ Prunode: Auto-Delete Dead Endpoint & Generate PR',
  ...
  let pruneDisposable = vscode.commands.registerCommand('prunode.prune', () => {
    ...
    vscode.window.showInformationMessage(
      'Prunode: Intelligently pruning dead code & imports...',
    );

    exec('prunode scan . --prune', { cwd: workspacePath }, (error, stdout) => {
      vscode.window.showInformationMessage(
        'Prunode: Cleaned up dead code successfully!',
      );
