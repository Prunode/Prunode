...
export function parseNextJsRoute(filePath: string, sourceCode: string, graph: PrunodeGraph, baseDir: string) {
    ...
    if (relativePath.includes('app/api/')) {
        routePath = '/api/' + relativePath.split('app/api/')[1].replace('/route.ts', '').replace('/route.js', '');
    } else if (relativePath.includes('pages/api/')) {
        routePath = '/api/' + relativePath.split('pages/api/')[1].replace('.ts', '').replace('.js', '').replace('/index', '');
    } else {
