...
export function parseNestJsFile(
  filePath: string,
  sourceCode: string,
  graph: PrunodeGraph,
) {
  ...
  function traverse(node: ts.Node) {
    if (ts.isClassDeclaration(node)) {
      ...
      const classDecorators = ts.getDecorators
        ...
        for (const dec of classDecorators) {
          if (ts.isCallExpression(dec.expression)) {
            if (
              ...
              dec.expression.expression.text === 'Controller'
            ) {
      ...
      if (basePath !== '') {
        ...
        for (const member of node.members) {
          if (ts.isMethodDeclaration(member)) {
            const methodDecorators = ts.getDecorators
                  ...
                  if (ts.isIdentifier(mDec.expression.expression)) {
                    ...
                    if (allowedMethods.includes(methodType)) {
                      ...
                      if (arg) {
                        const str = extractString(arg);
                        if (str)
                          routePath = str.startsWith('/') ? str : '/' + str;
                      }
