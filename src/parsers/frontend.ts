...
export function parseFrontendFile(filePath: string, sourceCode: string, graph: PrunodeGraph) {
    ...
    function extractString(node: ts.Node): string | null {
        ...
        if (ts.isTemplateExpression(node)) {
            ...
            for (const span of node.templateSpans) {
                fullString += '${' + span.expression.getText(sourceFile) + '}' + span.literal.text;
            }
            return fullString;
        }
        return null;
    ...
    function traverse(node: ts.Node, currentComponent?: string) {
        ...
        if (ts.isVariableDeclaration(node)) {
            ...
            if (node.initializer && ts.isCallExpression(node.initializer)) {
                ...
                if (ts.isPropertyAccessExpression(call.expression) && call.expression.expression.getText(sourceFile) === 'axios' && call.expression.name.text === 'create') {
                    const arg = call.arguments[0];
                    if (arg && ts.isObjectLiteralExpression(arg)) {
                        const baseURLProp = arg.properties.find(p => p.name && p.name.getText(sourceFile) === 'baseURL') as ts.PropertyAssignment;
        ...
        if (ts.isCallExpression(node)) {
            ...
            if (ts.isIdentifier(expression) && expression.text === 'fetch') {
                ...
                if (firstArg) {
                    ...
                    if (routeStr) {
                        ...
                        graph.addCall({ method: 'GET', path: routeStr, sourceFile: filePath, expectedKeys, componentName });
                    }
                }
            }
            ...
            if (ts.isPropertyAccessExpression(expression)) {
                ...
                if (['get', 'post', 'put', 'delete', 'patch'].includes(methodName)) {
                    ...
                    if (secondArg && (ts.isArrowFunction(secondArg) || ts.isFunctionExpression(secondArg))) {
                    ...
                    if (firstArg) {
                        ...
                        if (routeStr && (routeStr.startsWith('/') || routeStr.startsWith('http'))) {
                            const expectedKeys = findExpectedKeys(node);
                            graph.addCall({ method: methodName.toUpperCase(), path: routeStr, sourceFile: filePath, expectedKeys, componentName });
                        }
                    }
                }
            }
            ...
            if (ts.isPropertyAccessExpression(expression) && expression.name.text === 'emit') {
                if (ts.isIdentifier(expression.expression) && expression.expression.text === 'socket') {
                    ...
                    if (arg && ts.isStringLiteral(arg)) {
                        graph.addCall({ method: 'WS', path: arg.text, sourceFile: filePath, expectedKeys: [], componentName });
                    }
                }
            }
        }
